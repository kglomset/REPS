-- REPS seed data: workout history + body weight logs for admin@reps.dev
-- Run directly via psql: psql $DATABASE_URL -f seed_admin_data.sql
...
BEGIN
    ...
    IF admin_id IS NULL THEN RAISE EXCEPTION 'admin@reps.dev not found'; END IF;

    DELETE FROM workout_sessions WHERE user_id = admin_id;
    DELETE FROM body_weight_logs  WHERE user_id = admin_id;
    RAISE NOTICE 'Existing data wiped';
    ...
    RAISE NOTICE 'Done: 48 sessions, 36 body weight logs';
END $$;